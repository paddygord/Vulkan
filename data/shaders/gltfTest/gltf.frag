...
layout (set = 0, binding = 5) uniform sampler2D textures[16];

#define u_BaseColorSampler textures[0]
#define u_MetallicRoughnessSampler textures[1]
#define u_NormalSampler textures[2]
#define u_OcclusionSampler textures[3]
#define u_brdfLUT samplerBRDFLUT
#define u_DiffuseEnvSampler prefilteredMap
#define u_SpecularEnvSampler samplerIrradiance
#define u_LightDirection normalize(vec3(1.0, 1.0, -2.0))
#define u_LightColor vec3(1.0)


vec2 u_ScaleIBLAmbient = vec2(1.0, 0.0);

float u_NormalScale = 1.0;
float u_OcclusionStrength = 1.0;
vec2 u_MetallicRoughnessValues = vec2(1.0);
vec4 u_BaseColorFactor = vec4(1.0);
vec3 u_Camera = ubo.camPos;
vec3 v_Position = inWorldPos;
vec2 v_UV = inUV;
vec3 v_Normal = inNormal;
...
vec3 getIBLContribution(PBRInfo pbrInputs, vec3 n, vec3 reflection)
{
    ...
    vec3 diffuseLight = vec3(0.25); //SRGBtoLINEAR(texture(u_DiffuseEnvSampler, n)).rgb;
