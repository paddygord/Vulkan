...
namespace vkx {
    ...
    public:
        ...
        // Load a 2D texture
        void loadTexture(std::string filename, vk::Format format, Texture *texture, bool forceLinear) {
        ...
        void loadTexture(std::string filename, vk::Format format, Texture *texture, bool forceLinear, vk::ImageUsageFlags imageUsageFlags) {
            ...
            if (useStaging) {
                ...
                stagingBuffer = context.device.createBuffer(bufferCreateInfo);
                ...
                memAllocInfo.memoryTypeIndex = context.getMemoryType(memReqs.memoryTypeBits, vk::MemoryPropertyFlagBits::eHostVisible);

                stagingMemory = context.device.allocateMemory(memAllocInfo);
                context.device.bindBufferMemory(stagingBuffer, stagingMemory, 0);

                // Copy texture data into staging buffer
                void *data = context.device.mapMemory(stagingMemory, 0, memReqs.size, vk::MemoryMapFlags());
                ...
                texture->image = context.device.createImage(imageCreateInfo);
                ...
                texture->deviceMemory = context.device.allocateMemory(memAllocInfo);
                context.device.bindImageMemory(texture->image, texture->deviceMemory, 0);
                ...
                context.device.freeMemory(stagingMemory);
                ...
                mappableImage = context.device.createImage(imageCreateInfo);

                // Get memory requirements for this image 
                // like size and alignment
                memReqs = context.device.getImageMemoryRequirements(mappableImage);
                ...
                mappableMemory = context.device.allocateMemory(memAllocInfo);

                // Bind allocated image for use
                context.device.bindImageMemory(mappableImage, mappableMemory, 0);
                ...
                subResLayout = context.device.getImageSubresourceLayout(mappableImage, subRes);

                // Map image memory
                data = context.device.mapMemory(mappableMemory, 0, memReqs.size, vk::MemoryMapFlags());

                // Copy image data into memory
                memcpy(data, tex2D[subRes.mipLevel].data(), tex2D[subRes.mipLevel].size());

                context.device.unmapMemory(mappableMemory);
        ...
        void destroyTexture(Texture texture) {
            ...
            context.device.freeMemory(texture.deviceMemory);
        }
