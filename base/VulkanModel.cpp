...
const int vks::Model::defaultFlags = aiProcess_FlipWindingOrder | aiProcess_Triangulate | aiProcess_PreTransformVertices | aiProcess_CalcTangentSpace | aiProcess_GenSmoothNormals;

bool vks::Model::loadFromFile(const vkx::Context& context, const std::string& filename, const vks::VertexLayout& layout, vk::Optional<vks::ModelCreateInfo> createInfo, const int flags) {
...
bool vks::Model::loadFromFile(const vkx::Context& context, const std::string& filename, vks::VertexLayout layout, float scale, int flags) {
    return loadFromFile(context, filename, layout, vks::ModelCreateInfo{ scale, 1.0f, 0.0f }, flags);
}
