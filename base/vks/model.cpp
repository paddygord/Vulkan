...
bool Model::loadFromFile(const Context& context,
    ...
    for (unsigned int i = 0; i < pScene->mNumMeshes; i++) {
        ...
        for (unsigned int j = 0; j < pScene->mMeshes[i]->mNumVertices; j++) {
            appendVertex(vertexBuffer, pScene, i, j);
        }
...
bool Model::loadFromFile(const Context& context, const std::string& filename, const VertexLayout& layout, float scale, int flags) {
