...
public:
    ...
    void prepare(const vks::Context& context) {
        assert(!colorFormats.empty());
        assert(size != vk::Extent2D{});
        assert(depthFormat != vk::Format::eR8Uscaled);
        device = context.device;
        commandPool = context.getCommandPool();
        cmdBuffer = context.allocateCommandBuffer();
        renderComplete = device.createSemaphore(vk::SemaphoreCreateInfo{});

        prepareSampler();
        prepareRenderPass();
        for (auto& framebuffer : framebuffers) {
            framebuffer = vks::framebuffer::Builder{ context, renderPass, size }
                              .withColorFormats(colorFormats)
                              .withDepthFormat(depthFormat)
                              .withColorUsage(attachmentUsage)
                              .withDepthUsage(depthAttachmentUsage)
                              .create();
        }
    }
