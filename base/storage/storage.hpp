...
public:
    ...
    static StoragePointer create(size_t size, uint8_t* data = nullptr);
    static StoragePointer createFile(const std::string& filename, size_t size, const void* data = nullptr);

    template<typename T=uint8_t>
    static StoragePointer createFile(const std::string& filename, const std::vector<T>& data) {
