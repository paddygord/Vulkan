...
public:
    ...
    uint32_t layerCount;
    std::vector<vkx::texture::Texture2D> textures;
    ...
    void loadAssets() {
        ...
        for (uint32_t i = 0; i < 7; i++) {
            textures[i].loadFromFile(context, getAssetPath() + "textures/array/" + std::to_string(i) + ".ktx", vk::Format::eR8G8B8A8Unorm);
        }
    }
    ...
    void generateCube(glm::vec3 pos, float scale, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices) {
        ...
        std::vector<Vertex> cubeVertices = {
            { { -scale, -scale, scale }, { 0.0f, 0.0f }, faceTextureIndex[0] },  { { scale, -scale, scale }, { 1.0f, 0.0f }, faceTextureIndex[0] },
            { { scale, scale, scale }, { 1.0f, 1.0f }, faceTextureIndex[0] },    { { -scale, scale, scale }, { 0.0f, 1.0f }, faceTextureIndex[0] },

            { { scale, scale, scale }, { 0.0f, 0.0f }, faceTextureIndex[1] },    { { scale, scale, -scale }, { 1.0f, 0.0f }, faceTextureIndex[1] },
            { { scale, -scale, -scale }, { 1.0f, 1.0f }, faceTextureIndex[1] },  { { scale, -scale, scale }, { 0.0f, 1.0f }, faceTextureIndex[1] },

            { { -scale, -scale, -scale }, { 0.0f, 0.0f }, faceTextureIndex[2] }, { { scale, -scale, -scale }, { 1.0f, 0.0f }, faceTextureIndex[2] },
            { { scale, scale, -scale }, { 1.0f, 1.0f }, faceTextureIndex[2] },   { { -scale, scale, -scale }, { 0.0f, 1.0f }, faceTextureIndex[2] },

            { { -scale, -scale, -scale }, { 0.0f, 0.0f }, faceTextureIndex[3] }, { { -scale, -scale, scale }, { 1.0f, 0.0f }, faceTextureIndex[3] },
            { { -scale, scale, scale }, { 1.0f, 1.0f }, faceTextureIndex[3] },   { { -scale, scale, -scale }, { 0.0f, 1.0f }, faceTextureIndex[3] },

            { { scale, scale, scale }, { 0.0f, 0.0f }, faceTextureIndex[4] },    { { -scale, scale, scale }, { 1.0f, 0.0f }, faceTextureIndex[4] },
            { { -scale, scale, -scale }, { 1.0f, 1.0f }, faceTextureIndex[4] },  { { scale, scale, -scale }, { 0.0f, 1.0f }, faceTextureIndex[4] },

            { { -scale, -scale, -scale }, { 0.0f, 0.0f }, faceTextureIndex[5] }, { { scale, -scale, -scale }, { 1.0f, 0.0f }, faceTextureIndex[5] },
            { { scale, -scale, scale }, { 1.0f, 1.0f }, faceTextureIndex[5] },   { { -scale, -scale, scale }, { 0.0f, 1.0f }, faceTextureIndex[5] },
        ...
        std::vector<uint32_t> cubeIndices = { 0,  1,  2,  0,  2,  3,  4,  5,  6,  4,  6,  7,  8,  9,  10, 8,  10, 11,
	...
	*/
    void setupDescriptorSets() {
		...
		*/
        ...
        std::vector<vk::DescriptorSetLayoutBinding> setLayoutBindings = {
			...
			*/
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 1, static_cast<uint32_t>(textures.size()) }
        };
		...
		*/
        ...
        std::vector<vk::DescriptorBindingFlagsEXT> descriptorBindingFlags = { 0, VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT_EXT };
        setLayoutBindingFlags.pBindingFlags = descriptorBindingFlags.data();

        vk::DescriptorSetLayoutCreateInfo descriptorSetLayoutCI{ setLayoutBindings.data(), setLayoutBindings.size() };
		...
		*/
        ...
        std::vector<vk::DescriptorPoolSize> poolSizes = { { vk::DescriptorType::eUniformBuffer, 1 },
                                                          { vk::DescriptorType::eCombinedImageSampler, textures.size() } };
        vk::DescriptorPoolCreateInfo descriptorPoolInfo{ poolSizes.size(), poolSizes.data(), 2 };
		...
		*/
        ...
        writeDescriptorSets[0]{ descriptorSet, vk::DescriptorType::eUniformBuffer, 0, &uniformBufferVS.descriptor };
    ...
    void preparePipelines() {
        ...
        vk::PipelineRasterizationStateCreateInfo rasterizationStateCI{ VK_POLYGON_MODE_FILL, VK_CULL_MODE_NONE, VK_FRONT_FACE_COUNTER_CLOCKWISE, 0 };
        ...
        vk::VertexInputBindingDescription vertexInputBinding = { 0, sizeof(Vertex), vk::VertexInputRate::eVertex };
        std::vector<vk::VertexInputAttributeDescription> vertexInputAttributes = { { 0, 0, vk::Format::eR32G32B32sFloat, offsetof(Vertex, pos) },
                                                                                   { 1, 0, vk::Format::eR32G32sFloat, offsetof(Vertex, uv) },
                                                                                   { 2, 0, vk::Format::eR32_SINT, offsetof(Vertex, textureIndex) } };
        ...
        shaderStages[0] = loadShader(getAssetPath() + "shaders/descriptorindexing/descriptorindexing.vert.spv", vk::ShaderStageFlagBits::eVertex);
        shaderStages[1] = loadShader(getAssetPath() + "shaders/descriptorindexing/descriptorindexing.frag.spv", vk::ShaderStageFlagBits::eFragment);
