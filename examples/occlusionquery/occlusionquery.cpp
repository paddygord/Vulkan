...
public:
    ...
    struct {
        vkx::model::Model teapot;
        vkx::model::Model plane;
        vkx::model::Model sphere;
    } models;
    ...
    void buildCommandBuffers() {
        ...
        for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
            ...
            vk::Viewport viewport = vk::viewport{(float)width, (float)height, 0.0f, 1.0f};
            vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

            vk::Rect2D scissor = vk::rect2D{width, height, 0, 0};
    ...
    void loadAssets() {
        models.plane.loadFromFile(context, getAssetPath() + "models/plane_z.3ds", vertexLayout, 0.4f);
        models.teapot.loadFromFile(context, getAssetPath() + "models/teapot.3ds", vertexLayout, 0.3f);
        models.sphere.loadFromFile(context, getAssetPath() + "models/sphere.3ds", vertexLayout, 0.3f);
    }
    ...
    void setupDescriptorSets() {
        ...
        std::vector<vk::WriteDescriptorSet> writeDescriptorSets = {
            // Binding 0 : Vertex shader uniform buffer
            vk::writeDescriptorSet{descriptorSet, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.occluder.descriptor}
        };
    ...
    void prepareUniformBuffers() {
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.occluder,
                                                   sizeof(uboVS)));

        // Teapot
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.teapot,
                                                   sizeof(uboVS)));

        // Sphere
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.sphere,
                                                   sizeof(uboVS)));
