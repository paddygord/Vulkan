...
public:
    ...
    void renderNode(vkglTF::Node* node, vk::CommandBuffer commandBuffer) {
        if (node->mesh) {
            for (vkglTF::Primitive* primitive : node->mesh->primitives) {
                const std::vector<vk::DescriptorSet> descriptorsets = { descriptorSet, node->mesh->uniformBuffer.descriptorSet };
                ...
                vkCmdPushConstants(commandBuffer, pipelineLayout, vk::ShaderStageFlagBits::eVertex, 0, sizeof(PushBlock), &pushBlock);

                /*
					[POI] Setup the conditional rendering
				*/
                vk::ConditionalRenderingBeginInfoEXT conditionalRenderingBeginInfo{};
    ...
    void loadAssets() {
        scene.loadFromFile(context, getAssetPath() + "models/gltf/glTF-Embedded/Buggy.gltf");
    }
    ...
    void setupDescriptorSets() {
        ...
        vk::DescriptorPoolCreateInfo descriptorPoolCI{ poolSizes.size(), poolSizes.data(), 1 };
        ...
        std::array<vk::DescriptorSetLayout, 2> setLayouts = { descriptorSetLayout, scene.descriptorSetLayout };
        vk::PipelineLayoutCreateInfo pipelineLayoutCI{ setLayouts.data(), 2 };
        vk::PushConstantRange pushConstantRange{ vk::ShaderStageFlagBits::eVertex, sizeof(glm::vec4) * 2, 0 };
        ...
        std::vector<vk::WriteDescriptorSet> writeDescriptorSets = { { descriptorSet, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffer.descriptor } };
    ...
    void preparePipelines() {
        ...
        vk::PipelineRasterizationStateCreateInfo rasterizationStateCI{ VK_POLYGON_MODE_FILL, VK_CULL_MODE_BACK_BIT, VK_FRONT_FACE_COUNTER_CLOCKWISE, 0 };
        ...
        const std::vector<vk::VertexInputBindingDescription> vertexInputBindings = {
            { 0, sizeof(vkglTF::Model::Vertex), vk::VertexInputRate::eVertex },
        };
        const std::vector<vk::VertexInputAttributeDescription> vertexInputAttributes = {
            ...
            { 0, 2, vk::Format::eR32G32sFloat, sizeof(float) * 6 },     // Location 2: UV
        };
        ...
        const std::array<vk::PipelineShaderStageCreateInfo, 2> shaderStages = {
            loadShader(getAssetPath() + "shaders/conditionalrender/model.vert.spv", vk::ShaderStageFlagBits::eVertex),
            loadShader(getAssetPath() + "shaders/conditionalrender/model.frag.spv", vk::ShaderStageFlagBits::eFragment)
        };
	...
	*/
    void prepareConditionalRendering() {
		...
		*/
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT,
                                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &conditionalBuffer,
                                                   sizeof(int32_t) * conditionalVisibility.size(), conditionalVisibility.data()));
