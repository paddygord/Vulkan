...
public:
    ...
    void prepare() {
        ExampleBase::prepare();
        auto extensionsProperties = context.physicalDevice.enumerateDeviceExtensionProperties();
        for (const auto& extensionProperties : extensionsProperties) {
            OutputDebugStringA(extensionProperties.extensionName);
            OutputDebugStringA("\n");
        }
        dispatcher.init(context.instance, context.device);
        auto properties2 = context.physicalDevice.getProperties2KHR<vk::PhysicalDeviceProperties2, vk::PhysicalDeviceMultiviewProperties>(dispatcher);
        auto features2 = context.physicalDevice.getFeatures2KHR<vk::PhysicalDeviceFeatures2, vk::PhysicalDeviceMultiviewFeatures>(dispatcher);
