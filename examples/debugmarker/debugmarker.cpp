...
public:
    ...
    void loadScene() {
        scene.loadFromFile(context, getAssetPath() + "models/treasure_smooth.dae");
        sceneGlow.loadFromFile(context, getAssetPath() + "models/treasure_glow.dae");
        ...
        std::vector<std::string> names = {
            "hill",         "crystals",  "rocks",         "cave", "tree", "mushroom stems", "blue mushroom caps", "red mushroom caps",
			...
			*/
            {
                ...
                vk::Rect2D scissor{ wireframe ? width / 2 : width, height, 0, 0 };
                vkCmdSetScissor(drawCmdBuffers[i], 0, 1, &scissor);
    ...
    void setupDescriptorPool() {
        ...
        vk::DescriptorPoolCreateInfo descriptorPoolInfo{ poolSizes.size(), poolSizes.data(), 1 };
    ...
    void setupDescriptorSetLayout() {
        ...
        DebugMarker::setObjectName(device, (uint64_t)descriptorSetLayout, VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT_EXT,
    ...
    void setupDescriptorSet() {
        ...
        std::vector<vk::WriteDescriptorSet> writeDescriptorSets = {
            // Binding 0 : Vertex shader uniform buffer
            { descriptorSets.scene, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffer.descriptor },
            // Binding 1 : Color map
            { descriptorSets.scene, vk::DescriptorType::eCombinedImageSampler, 1, &offscreenPass.descriptor }
        };
    ...
    void preparePipelines() {
        ...
        std::vector<vk::VertexInputAttributeDescription> vertexInputAttributes = {
            ...
            { 1, 0, vk::Format::eR32G32B32sFloat, sizeof(float) * 3 },  // Location 1: Normal
            { 2, 0, vk::Format::eR32G32sFloat, sizeof(float) * 6 },     // Location 2: Texture coordinates
            { 3, 0, vk::Format::eR32G32B32sFloat, sizeof(float) * 8 },  // Location 3: Color
        };
        ...
        DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 0], VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
                                   "Toon shading vertex shader");
        DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 1], VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
        ...
        DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 3], VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
                                   "Color-only fragment shader");
        DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 4], VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
                                   "Postprocess vertex shader");
        DebugMarker::setObjectName(device, (uint64_t)shaderModules[moduleIndex + 5], VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
