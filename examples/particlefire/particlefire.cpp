...
public:
    struct {
        struct {
            vkx::texture::Texture2D smoke;
            vkx::texture::Texture2D fire;
    ...
    void prepareParticles() {
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, particles.size,
                                                   &particles.buffer, &particles.memory, particleBuffer.data()));
    ...
    void loadAssets() {
        ...
        // Particles
        textures.particles.smoke.loadFromFile(context, getAssetPath() + "textures/particle_smoke.ktx", vk::Format::eR8G8B8A8Unorm);
        textures.particles.fire.loadFromFile(context, getAssetPath() + "textures/particle_fire.ktx", vk::Format::eR8G8B8A8Unorm);

        // Floor
        textures.floor.colorMap.loadFromFile(context, getAssetPath() + "textures/fireplace_colormap" + texFormatSuffix + ".ktx", texFormat);
        textures.floor.normalMap.loadFromFile(context, getAssetPath() + "textures/fireplace_normalmap" + texFormatSuffix + ".ktx", texFormat);
        ...
        models.environment.loadFromFile(context, getAssetPath() + "models/fireplace.obj", vertexLayout, 10.0f);
    }

    void setupDescriptorPool() {
        // Example uses one ubo and one image sampler
        std::vector<vk::DescriptorPoolSize> poolSizes = { { vk::DescriptorType::eUniformBuffer, 2 }, { vk::DescriptorType::eCombinedImageSampler, 4 } };
    ...
    void setupDescriptorSets() {
        ...
        vk::DescriptorImageInfo texDescriptorSmoke =
            vk::descriptorImageInfo{textures.particles.sampler, textures.particles.smoke.view, VK_IMAGE_LAYOUT_GENERAL};
        vk::DescriptorImageInfo texDescriptorFire =
            vk::descriptorImageInfo{textures.particles.sampler, textures.particles.fire.view, VK_IMAGE_LAYOUT_GENERAL};

        writeDescriptorSets = {
            // Binding 0: Vertex shader uniform buffer
            vk::writeDescriptorSet{descriptorSets.particles, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.fire.descriptor},
            // Binding 1: Smoke texture
            vk::writeDescriptorSet{descriptorSets.particles, vk::DescriptorType::eCombinedImageSampler, 1, &texDescriptorSmoke},
            // Binding 1: Fire texture array
            vk::writeDescriptorSet{descriptorSets.particles, vk::DescriptorType::eCombinedImageSampler, 2, &texDescriptorFire}
        };
        ...
        writeDescriptorSets = {
            // Binding 0: Vertex shader uniform buffer
            vk::writeDescriptorSet{descriptorSets.environment, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.environment.descriptor},
            // Binding 1: Color map
            vks::initializers::writeDescriptorSet(descriptorSets.environment, vk::DescriptorType::eCombinedImageSampler, 1,
                                                  &textures.floor.colorMap.descriptor),
            // Binding 2: Normal map
            vks::initializers::writeDescriptorSet(descriptorSets.environment, vk::DescriptorType::eCombinedImageSampler, 2,
    ...
    void preparePipelines() {
        ...
        // Particle rendering pipeline
        {
            ...
            // Vertex input state
            vk::VertexInputBindingDescription vertexInputBinding = { VERTEX_BUFFER_BIND_ID, sizeof(Particle), vk::VertexInputRate::eVertex };

            std::vector<vk::VertexInputAttributeDescription> vertexInputAttributes = {
                { VERTEX_BUFFER_BIND_ID, 0, vk::Format::eR32G32B32A32sFloat, offsetof(Particle, pos) },    // Location 0: Position
                { VERTEX_BUFFER_BIND_ID, 1, vk::Format::eR32G32B32A32sFloat, offsetof(Particle, color) },  // Location 1: Color
                { VERTEX_BUFFER_BIND_ID, 2, vk::Format::eR32sFloat, offsetof(Particle, alpha) },           // Location 2: Alpha
                { VERTEX_BUFFER_BIND_ID, 3, vk::Format::eR32sFloat, offsetof(Particle, size) },            // Location 3: Size
                { VERTEX_BUFFER_BIND_ID, 4, vk::Format::eR32sFloat, offsetof(Particle, rotation) },        // Location 4: Rotation
                { VERTEX_BUFFER_BIND_ID, 5, vk::Format::eR32_SINT, offsetof(Particle, type) },             // Location 5: Particle type
            };
        ...
        // Environment rendering pipeline (normal mapped)
        {
            ...
            // Vertex input state
            vk::VertexInputBindingDescription vertexInputBinding = { VERTEX_BUFFER_BIND_ID, vertexLayout.stride(), vk::VertexInputRate::eVertex };

            std::vector<vk::VertexInputAttributeDescription> vertexInputAttributes = {
                { VERTEX_BUFFER_BIND_ID, 0, vk::Format::eR32G32B32sFloat, 0 },                   // Location 0: Position
                { VERTEX_BUFFER_BIND_ID, 1, vk::Format::eR32G32sFloat, sizeof(float) * 3 },      // Location 1: UV
                ...
                { VERTEX_BUFFER_BIND_ID, 4, vk::Format::eR32G32B32sFloat, sizeof(float) * 11 },  // Location 4: Bitangen
            };
    ...
    void prepareUniformBuffers() {
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.fire,
                                                   sizeof(uboVS)));

        // Vertex shader uniform buffer block
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.environment,
                                                   sizeof(uboEnv)));
