...
class VulkanExample : public vkx::ExampleBase {
    float transform_time{ 0.2f };
    bool transform_animate{ 1 };
    float palette_factor{ 0 };
    int palette_image_idx{ 0 };
    unsigned int seed{ 23232323 };
    size_t batch_size{ k_Def_Batch_Size };
    size_t point_count{ k_Def_Point_Count };
    bool windowed{ 1 };
    bool verbose{ 0 };
    size_t cpu_core_count{ std::thread::hardware_concurrency() };
    std::array<graph_data_t, MAX_CPU_CORES> graph_data;
...
public:
    ...
    void updateDrawCommandBuffer(const vk::CommandBuffer& drawCommand) override {
    }
