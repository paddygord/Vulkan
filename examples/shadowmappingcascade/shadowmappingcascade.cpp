...
public:
    ...
    std::vector<vkx::model::Model> models;

    struct Material {
        vkx::texture::Texture2D texture;
	...
	*/
    void renderScene(vk::CommandBuffer commandBuffer, vk::PipelineLayout pipelineLayout, vk::DescriptorSet descriptorSet, uint32_t cascadeIndex = 0) {
	...
	*/
    void prepareDepthPass() {
        vk::Format depthFormat = vulkanDevice->getSupportedDepthFormat(true);
			...
			*/
            {
                ...
                vk::Viewport viewport{ (float)SHADOWMAP_DIM, (float)SHADOWMAP_DIM, 0.0f, 1.0f };
                vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

                vk::Rect2D scissor{ SHADOWMAP_DIM, SHADOWMAP_DIM, 0, 0 };
			...
			*/

            {
                ...
                if (displayDepthMap) {
                    ...
                    vkCmdPushConstants(drawCmdBuffers[i], pipelineLayout, vk::ShaderStageFlagBits::eVertex, 0, sizeof(PushConstBlock), &pushConstBlock);
    ...
    void loadAssets() {
        materials.resize(3);
        materials[0].texture.loadFromFile(context, getAssetPath() + "textures/gridlines.ktx", vk::Format::eR8G8B8A8Unorm);
        materials[1].texture.loadFromFile(context, getAssetPath() + "textures/oak_bark.ktx", vk::Format::eR8G8B8A8Unorm);
        materials[2].texture.loadFromFile(context, getAssetPath() + "textures/oak_leafs.ktx", vk::Format::eR8G8B8A8Unorm);
    ...
    void setupLayoutsAndDescriptors() {
        /*
			Descriptor pool
		*/
        std::vector<vk::DescriptorPoolSize> poolSizes = { { vk::DescriptorType::eUniformBuffer, 32 }, { vk::DescriptorType::eCombinedImageSampler, 32 } };
        vk::DescriptorPoolCreateInfo descriptorPoolInfo = { static_cast<uint32_t>(poolSizes.size()), poolSizes.data(), 4 + SHADOW_MAP_CASCADE_COUNT };
		...
		*/
        ...
        descriptorLayout{ setLayoutBindings };
		...
		*/
        ...
        vk::DescriptorImageInfo depthMapDescriptor = { depth.sampler, depth.view, VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL };

        vk::DescriptorSetAllocateInfo allocInfo = { descriptorPool, &descriptorSetLayouts.base, 1 };

        // Scene rendering / debug display
        VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &descriptorSet));
        writeDescriptorSets = {
            { descriptorSet, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.VS.descriptor },
            { descriptorSet, vk::DescriptorType::eCombinedImageSampler, 1, &depthMapDescriptor },
            { descriptorSet, vk::DescriptorType::eUniformBuffer, 2, &uniformBuffers.FS.descriptor },
        };
        ...
        for (uint32_t i = 0; i < SHADOW_MAP_CASCADE_COUNT; i++) {
            ...
            vk::DescriptorImageInfo cascadeImageInfo{ depth.sampler, depth.view, VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL };
            writeDescriptorSets = { { cascades[i].descriptorSet, vk::DescriptorType::eUniformBuffer, 0, &depthPass.uniformBuffer.descriptor },
                                    { cascades[i].descriptorSet, vk::DescriptorType::eCombinedImageSampler, 1, &cascadeImageInfo } };
            vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
        }
        ...
        for (auto& material : materials) {
            ...
            vk::WriteDescriptorSet writeDescriptorSet{ material.descriptorSet, vk::DescriptorType::eCombinedImageSampler, 0, &material.texture.descriptor };
		...
		*/
        ...
        // Shared pipeline layout (scene and depth map debug display)
        {
            ...
            vk::PipelineLayoutCreateInfo pipelineLayoutCreateInfo{ setLayouts.data(), static_cast<uint32_t>(setLayouts.size()) };
    ...
    void preparePipelines() {
        ...
        shaderStages[0] = loadShader(getAssetPath() + "shaders/shadowmappingcascade/debugshadowmap.vert.spv", vk::ShaderStageFlagBits::eVertex);
        shaderStages[1] = loadShader(getAssetPath() + "shaders/shadowmappingcascade/debugshadowmap.frag.spv", vk::ShaderStageFlagBits::eFragment);
		...
		*/
        ...
        shaderStages[0] = loadShader(getAssetPath() + "shaders/shadowmappingcascade/scene.vert.spv", vk::ShaderStageFlagBits::eVertex);
        shaderStages[1] = loadShader(getAssetPath() + "shaders/shadowmappingcascade/scene.frag.spv", vk::ShaderStageFlagBits::eFragment);
		...
		*/
        shaderStages[0] = loadShader(getAssetPath() + "shaders/shadowmappingcascade/depthpass.vert.spv", vk::ShaderStageFlagBits::eVertex);
        shaderStages[1] = loadShader(getAssetPath() + "shaders/shadowmappingcascade/depthpass.frag.spv", vk::ShaderStageFlagBits::eFragment);
    ...
    void prepareUniformBuffers() {
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &depthPass.uniformBuffer,
