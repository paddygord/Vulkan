...
public:
    ...
    void preparePipelines()
    {
        ...
        shaderStages[0] = context.loadShader(getAssetPath() + "shaders/hdr/composition.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
        shaderStages[1] = context.loadShader(getAssetPath() + "shaders/hdr/composition.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
        ...
        VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, context.pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.composition));

        // Bloom pass
        shaderStages[0] = context.loadShader(getAssetPath() + "shaders/hdr/bloom.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
        shaderStages[1] = context.loadShader(getAssetPath() + "shaders/hdr/bloom.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
        ...
        VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, context.pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.bloom[0]));

        // Second blur pass (into separate framebuffer)
        pipelineCreateInfo.renderPass = filterPass.renderPass;
        dir = 0;
        VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, context.pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.bloom[1]));
