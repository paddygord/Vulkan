...
public:
        ...
        for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
...

            {
				...
				*/
                ...
                vk::Viewport viewport{ (float)offscreen.width, (float)offscreen.height, 0.0f, 1.0f };
                vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

                vk::Rect2D scissor{ offscreen.width, offscreen.height, 0, 0 };
			...
			*/
            if (bloom) {
                ...
                vk::Viewport viewport{ (float)filterPass.width, (float)filterPass.height, 0.0f, 1.0f };
                vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

                vk::Rect2D scissor{ filterPass.width, filterPass.height, 0, 0 };
                vkCmdSetScissor(drawCmdBuffers[i], 0, 1, &scissor);

                vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.bloomFilter, 0, 1, &descriptorSets.bloomFilter, 0,
                                        NULL);
    ...
    void prepareoffscreenfer() {
        {
            ...
            createAttachment(vk::Format::eR32G32B32A32sFloat, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &offscreen.color[0]);
            createAttachment(vk::Format::eR32G32B32A32sFloat, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &offscreen.color[1]);
            // Depth attachment
        ...
        // Bloom separable filter pass
        {
            ...
            // Two floating point color buffers
            createAttachment(vk::Format::eR32G32B32A32sFloat, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &filterPass.color[0]);
    ...
    void setupDescriptorPool() {
        ...
        vk::DescriptorPoolCreateInfo descriptorPoolInfo = { static_cast<uint32_t>(poolSizes.size()), poolSizes.data(), numDescriptorSets };
    ...
    void setupDescriptorSetLayout() {
        ...
        vk::DescriptorSetLayoutCreateInfo descriptorLayoutInfo = { setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()) };

        VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device, &descriptorLayoutInfo, nullptr, &descriptorSetLayouts.models));

        vk::PipelineLayoutCreateInfo pipelineLayoutCreateInfo = vk::pipelineLayoutCreateInfo{&descriptorSetLayouts.models, 1};
        ...
        pipelineLayoutCreateInfo{ &descriptorSetLayouts.bloomFilter, 1 };
        ...
        pipelineLayoutCreateInfo{ &descriptorSetLayouts.composition, 1 };
        VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pipelineLayoutCreateInfo, nullptr, &pipelineLayouts.composition));
    }

    void setupDescriptorSets() {
        vk::DescriptorSetAllocateInfo allocInfo = vk::descriptorSetAllocateInfo{descriptorPool, &descriptorSetLayouts.models, 1};

        // 3D object descriptor set
        VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &descriptorSets.object));

        std::vector<vk::WriteDescriptorSet> writeDescriptorSets = {
            { descriptorSets.object, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.matrices.descriptor },
            { descriptorSets.object, vk::DescriptorType::eCombinedImageSampler, 1, &textures.envmap.descriptor },
            { descriptorSets.object, vk::DescriptorType::eUniformBuffer, 2, &uniformBuffers.params.descriptor },
        };
        ...
        writeDescriptorSets = {
            { descriptorSets.skybox, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.matrices.descriptor },
            { descriptorSets.skybox, vk::DescriptorType::eCombinedImageSampler, 1, &textures.envmap.descriptor },
            { descriptorSets.skybox, vk::DescriptorType::eUniformBuffer, 2, &uniformBuffers.params.descriptor },
        };
        vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);

        // Bloom filter
        allocInfo{ descriptorPool, &descriptorSetLayouts.bloomFilter, 1 };
        ...
        std::vector<vk::DescriptorImageInfo> colorDescriptors = {
            { offscreen.sampler, offscreen.color[0].view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
            { offscreen.sampler, offscreen.color[1].view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
        };

        writeDescriptorSets = {
            { descriptorSets.bloomFilter, vk::DescriptorType::eCombinedImageSampler, 0, &colorDescriptors[0] },
            { descriptorSets.bloomFilter, vk::DescriptorType::eCombinedImageSampler, 1, &colorDescriptors[1] },
        };
        vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);

        // Composition descriptor set
        allocInfo = { descriptorPool, &descriptorSetLayouts.composition, 1 };
        VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &descriptorSets.composition));

        colorDescriptors = {
            { offscreen.sampler, offscreen.color[0].view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
            { offscreen.sampler, filterPass.color[0].view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
        };

        writeDescriptorSets = {
            { descriptorSets.composition, vk::DescriptorType::eCombinedImageSampler, 0, &colorDescriptors[0] },
            { descriptorSets.composition, vk::DescriptorType::eCombinedImageSampler, 1, &colorDescriptors[1] },
        };
        vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
    }
    ...
    void preparePipelines() {
        ...
        vk::GraphicsPipelineCreateInfo pipelineCreateInfo = vk::pipelineCreateInfo{pipelineLayouts.models, renderPass, 0};
        ...
        specializationMapEntries[0]{ 0, 0, sizeof(uint32_t) };
        uint32_t dir = 1;
        specializationInfo{ 1, specializationMapEntries.data(), sizeof(dir), &dir };
        ...
        specializationInfo{ 1, specializationMapEntries.data(), sizeof(shadertype), &shadertype };
    ...
    void prepareUniformBuffers() {
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.matrices,
                                                   sizeof(uboVS)));
