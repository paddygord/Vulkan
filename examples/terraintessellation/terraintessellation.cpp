...
public:
    ...
    struct {
        vkx::texture::Texture2D heightMap;
        vkx::texture::Texture2D skySphere;
        vkx::texture::Texture2DArray terrainArray;
    } textures;
    ...
    void setupQueryResultBuffer() {
        ...
        vk::BufferCreateInfo bufferCreateInfo =
            vk::bufferCreateInfo{VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, bufSize};
    ...
    void loadAssets() {
        models.skysphere.loadFromFile(context, getAssetPath() + "models/geosphere.obj", vertexLayout, 1.0f);
        ...
        textures.skySphere.loadFromFile(context, getAssetPath() + "textures/skysphere" + texFormatSuffix + ".ktx", texFormat);
        // Terrain textures are stored in a texture array with layers corresponding to terrain height
        textures.terrainArray.loadFromFile(context, getAssetPath() + "textures/terrain_texturearray" + texFormatSuffix + ".ktx", texFormat);
...
#define UV_SCALE 1.0f
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                                   vertexBufferSize, &vertexStaging.buffer, &vertexStaging.memory, vertices));
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                                   vertexBufferSize, &models.terrain.vertices.buffer, &models.terrain.vertices.memory));
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                                   indexBufferSize, &models.terrain.indices.buffer, &models.terrain.indices.memory));
        ...
        vkCmdCopyBuffer(copyCmd, vertexStaging.buffer, models.terrain.vertices.buffer, 1, &copyRegion);

        copyRegion.size = indexBufferSize;
        vkCmdCopyBuffer(copyCmd, indexStaging.buffer, models.terrain.indices.buffer, 1, &copyRegion);
    ...
    void setupDescriptorPool() {
        std::vector<vk::DescriptorPoolSize> poolSizes = { { vk::DescriptorType::eUniformBuffer, 3 }, { vk::DescriptorType::eCombinedImageSampler, 3 } };
    ...
    void setupDescriptorSetLayouts() {
        ...
        setLayoutBindings = {
            ...
            vks::initializers::descriptorSetLayoutBinding(vk::DescriptorType::eUniformBuffer,
                                                          vk::ShaderStageFlagBits::eTESSELLATION_CONTROL | vk::ShaderStageFlagBits::eTESSELLATION_EVALUATION,
                                                          0),
        ...
        pipelineLayoutCreateInfo{ &descriptorSetLayouts.terrain, 1 };
        ...
        pipelineLayoutCreateInfo{ &descriptorSetLayouts.skysphere, 1 };
    ...
    void setupDescriptorSets() {
        ...
        // Terrain
        allocInfo{ descriptorPool, &descriptorSetLayouts.terrain, 1 };
        VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &descriptorSets.terrain));

        writeDescriptorSets = {
            // Binding 0 : Shared tessellation shader ubo
            vks::initializers::writeDescriptorSet(descriptorSets.terrain, vk::DescriptorType::eUniformBuffer, 0,
                                                  &uniformBuffers.terrainTessellation.descriptor),
            // Binding 1 : Displacement map
            vk::writeDescriptorSet{descriptorSets.terrain, vk::DescriptorType::eCombinedImageSampler, 1, &textures.heightMap.descriptor},
            // Binding 2 : Color map (alpha channel)
            vk::writeDescriptorSet{descriptorSets.terrain, vk::DescriptorType::eCombinedImageSampler, 2, &textures.terrainArray.descriptor},
        };
        vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);

        // Skysphere
        allocInfo{ descriptorPool, &descriptorSetLayouts.skysphere, 1 };
        VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &descriptorSets.skysphere));

        writeDescriptorSets = {
            // Binding 0 : Vertex shader ubo
            vk::writeDescriptorSet{descriptorSets.skysphere, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.skysphereVertex.descriptor},
            // Binding 1 : Fragment shader color map
            vk::writeDescriptorSet{descriptorSets.skysphere, vk::DescriptorType::eCombinedImageSampler, 1, &textures.skySphere.descriptor},
        };
        vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
    }
    ...
    void preparePipelines() {
        ...
        shaderStages[2] = loadShader(getAssetPath() + "shaders/terraintessellation/terrain.tesc.spv", vk::ShaderStageFlagBits::eTESSELLATION_CONTROL);
        shaderStages[3] = loadShader(getAssetPath() + "shaders/terraintessellation/terrain.tese.spv", vk::ShaderStageFlagBits::eTESSELLATION_EVALUATION);

        vk::GraphicsPipelineCreateInfo pipelineCreateInfo = { pipelineLayouts.terrain, renderPass, 0 };
    ...
    void prepareUniformBuffers() {
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &uniformBuffers.skysphereVertex,
                                                   sizeof(uboVS)));
