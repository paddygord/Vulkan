...
private:
    ...
    void loadMaterials() {
        ...
        std::vector<vk::DescriptorPoolSize> poolSizes;
                poolSizes.push_back({ vk::DescriptorType::eUniformBuffer, static_cast<uint32_t>(materials.size())) };
		poolSizes.push_back({ vk::DescriptorType::eCombinedImageSampler, static_cast<uint32_t>(materials.size())) };
		...
		{
            // Descriptor set
            vk::DescriptorSetAllocateInfo allocInfo = vk::descriptorSetAllocateInfo{descriptorPool, &descriptorSetLayouts.material, 1};
            ...
            writeDescriptorSets.push_back(vks::initializers::writeDescriptorSet(materials[i].descriptorSet, vk::DescriptorType::eCombinedImageSampler, 0,
    ...
    void loadMeshes(vk::CommandBuffer copyCmd) {
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                                   &vertexStaging, static_cast<uint32_t>(vertexDataSize), vertices.data()));
        // Target
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                                   &vertexBuffer, static_cast<uint32_t>(vertexDataSize)));
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                                   &indexBuffer, static_cast<uint32_t>(indexDataSize)));
...
public:
    ...
    Scene(vks::VulkanDevice* vulkanDevice, vk::Queue queue) {
        ...
        vk::BufferCreateInfo bufferCreateInfo{ VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, sizeof(uniformData) };
    ...
    void render(vk::CommandBuffer cmdBuffer, bool wireframe) {
        ...
        for (size_t i = 0; i < meshes.size(); i++) {
            ...
            vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, static_cast<uint32_t>(descriptorSets.size()),
                                    descriptorSets.data(), 0, NULL);

            // Pass material properies via push constants
            vkCmdPushConstants(cmdBuffer, pipelineLayout, vk::ShaderStageFlagBits::eFragment, 0, sizeof(SceneMaterialProperties),
...
public:
    ...
    void preparePipelines() {
        ...
        vk::GraphicsPipelineCreateInfo pipelineCreateInfo = vk::pipelineCreateInfo{scene->pipelineLayout, renderPass, 0};
    ...
    void loadScene() {
        vk::CommandBuffer copyCmd = VulkanExampleBase::createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, false);
