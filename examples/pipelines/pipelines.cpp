...
public:
    ...
    void preparePipelines() {
        ...
        shaderStages[0] = context.loadShader(getAssetPath() + "shaders/pipelines/phong.vert.spv", vk::ShaderStageFlagBits::eVertex);
        shaderStages[1] = context.loadShader(getAssetPath() + "shaders/pipelines/phong.frag.spv", vk::ShaderStageFlagBits::eFragment);
        ...
        // Textured pipeline
        pipelines.phong = device.createGraphicsPipelines(context.pipelineCache, pipelineCreateInfo, nullptr)[0];
        ...
        shaderStages[0] = context.loadShader(getAssetPath() + "shaders/pipelines/toon.vert.spv", vk::ShaderStageFlagBits::eVertex);
        shaderStages[1] = context.loadShader(getAssetPath() + "shaders/pipelines/toon.frag.spv", vk::ShaderStageFlagBits::eFragment);

        pipelines.toon = device.createGraphicsPipelines(context.pipelineCache, pipelineCreateInfo, nullptr)[0];

        // Non solid rendering is not a mandatory Vulkan feature
        if (context.deviceFeatures.fillModeNonSolid) {
            // vk::Pipeline for wire frame rendering
            rasterizationState.polygonMode = vk::PolygonMode::eLine;
            shaderStages[0] = context.loadShader(getAssetPath() + "shaders/pipelines/wireframe.vert.spv", vk::ShaderStageFlagBits::eVertex);
            shaderStages[1] = context.loadShader(getAssetPath() + "shaders/pipelines/wireframe.frag.spv", vk::ShaderStageFlagBits::eFragment);
