...
vkx::vertex::Layout vertexLayout{ {
    ...
    vkx::vertex::VERTEX_COMPONENT_DUMMY_VEC4,
    vkx::vertex::VERTEX_COMPONENT_DUMMY_VEC4,
} };
...
public:
    ...
    void loadMesh() {
        ...
        for (uint32_t m = 0; m < skinnedMesh->scene->mNumMeshes; m++) {
            for (uint32_t v = 0; v < skinnedMesh->scene->mMeshes[m]->mNumVertices; v++) {
                ...
                vertex.color =
                    (skinnedMesh->scene->mMeshes[m]->HasVertexColors(0)) ? glm::make_vec3(&skinnedMesh->scene->mMeshes[m]->mColors[0][v].r) : glm::vec3(1.0f);
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                                   &skinnedMesh->vertexBuffer.indices, indexBufferSize));
    ...
    void loadAssets() {
        models.floor.loadFromFile(context, getAssetPath() + "models/plane_z.obj", vertexLayout, 512.0f);
        ...
        textures.colorMap.loadFromFile(context, getAssetPath() + "textures/goblin" + texFormatSuffix + ".ktx", texFormat);
        textures.floor.loadFromFile(context, getAssetPath() + "textures/trail" + texFormatSuffix + ".ktx", texFormat);
    }
    ...
    void setupDescriptorSet() {
        ...
        vk::DescriptorImageInfo texDescriptor =
            vk::descriptorImageInfo{textures.colorMap.sampler, textures.colorMap.view, VK_IMAGE_LAYOUT_GENERAL};

        std::vector<vk::WriteDescriptorSet> writeDescriptorSets = {
            // Binding 0 : Vertex shader uniform buffer
            vk::writeDescriptorSet{descriptorSet, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.mesh.descriptor},
            // Binding 1 : Color map
            vk::writeDescriptorSet{descriptorSet, vk::DescriptorType::eCombinedImageSampler, 1, &texDescriptor}
        };
        ...
        writeDescriptorSets.push_back(
            vk::writeDescriptorSet{descriptorSets.floor, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.floor.descriptor)};
        // Binding 1 : Color map
        writeDescriptorSets.push_back(
            vk::writeDescriptorSet{descriptorSets.floor, vk::DescriptorType::eCombinedImageSampler, 1, &texDescriptor)};
    ...
    void preparePipelines() {
        ...
        // Binding description
        vk::VertexInputBindingDescription vertexInputBinding = { VERTEX_BUFFER_BIND_ID, sizeof(Vertex), vk::VertexInputRate::eVertex };
        ...
        std::vector<vk::VertexInputAttributeDescription> vertexInputAttributes = {
            ...
            { VERTEX_BUFFER_BIND_ID, 4, vk::Format::eR32G32B32A32sFloat, sizeof(float) * 11 },  // Location 4: Bone weights
            { VERTEX_BUFFER_BIND_ID, 5, vk::Format::eR32G32B32A32_SINT, sizeof(float) * 15 },   // Location 5: Bone IDs
        };
