...
public:
    ...
    void prepareMultiThreadedRenderer() {
        ...
        vk::CommandBufferAllocateInfo cmdBufAllocateInfo = vk::commandBufferAllocateInfo{cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, 1};
        ...
        for (uint32_t i = 0; i < numThreads; i++) {
            ...
            vk::CommandBufferAllocateInfo secondaryCmdBufAllocateInfo =
                vk::commandBufferAllocateInfo{thread->commandPool, VK_COMMAND_BUFFER_LEVEL_SECONDARY, thread->commandBuffer.size()};
    ...
    void threadRenderCode(uint32_t threadIndex, uint32_t cmdBufferIndex, vk::CommandBufferInheritanceInfo inheritanceInfo) {
        ...
        objectData->model =
            glm::rotate(objectData->model, -sinf(glm::radians(objectData->deltaT * 360.0f)) * 0.25f, glm::vec3(objectData->rotationDir, 0.0f, 0.0f));
        ...
        vkCmdPushConstants(cmdBuffer, pipelineLayout, vk::ShaderStageFlagBits::eVertex, 0, sizeof(ThreadPushConstantBlock),
    ...
    void updateSecondaryCommandBuffers(vk::CommandBufferInheritanceInfo inheritanceInfo) {
		...
		*/
        ...
        vkCmdPushConstants(secondaryCommandBuffers.background, pipelineLayout, vk::ShaderStageFlagBits::eVertex, 0, sizeof(mvp), &mvp);
    ...
    void loadAssets() {
        models.ufo.loadFromFile(context, getAssetPath() + "models/retroufo_red_lowpoly.dae", vertexLayout, 0.12f);
        models.skysphere.loadFromFile(context, getAssetPath() + "models/sphere.obj", vertexLayout, 1.0f);
        objectSphereDim = std::max(std::max(models.ufo.dim.size.x, models.ufo.dim.size.y), models.ufo.dim.size.z);
    }

    void setupPipelineLayout() {
        vk::PipelineLayoutCreateInfo pPipelineLayoutCreateInfo = { nullptr, 0 };

        // Push constants for model matrices
        vk::PushConstantRange pushConstantRange = vk::pushConstantRange{vk::ShaderStageFlagBits::eVertex, sizeof(ThreadPushConstantBlock), 0};
