...
public:
    ...
    void prepareOffscreenFramebuffer() {
        ...
        createAttachment(vk::Format::eR16G16B16A16sFloat, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &offScreenFrameBuf.position);

        // (World space) Normals
        createAttachment(vk::Format::eR16G16B16A16sFloat, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &offScreenFrameBuf.normal);

        // Albedo (color)
        createAttachment(vk::Format::eR8G8B8A8Unorm, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &offScreenFrameBuf.albedo);
    ...
    void buildDeferredCommandBuffer() {
        ...
        vk::Viewport viewport{ (float)offScreenFrameBuf.width, (float)offScreenFrameBuf.height, 0.0f, 1.0f };
        vkCmdSetViewport(offScreenCmdBuffer, 0, 1, &viewport);

        vk::Rect2D scissor{ offScreenFrameBuf.width, offScreenFrameBuf.height, 0, 0 };
    ...
    void setupDescriptorPool() {
        std::vector<vk::DescriptorPoolSize> poolSizes = { { vk::DescriptorType::eUniformBuffer, 8 }, { vk::DescriptorType::eCombinedImageSampler, 9 } };

        vk::DescriptorPoolCreateInfo descriptorPoolInfo =
            vk::descriptorPoolCreateInfo{static_cast<uint32_t>(poolSizes.size()), poolSizes.data(), 3};
    ...
    void setupDescriptorSet() {
        ...
        vk::DescriptorImageInfo texDescriptorPosition =
            vk::descriptorImageInfo{colorSampler, offScreenFrameBuf.position.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};

        vk::DescriptorImageInfo texDescriptorNormal =
            vk::descriptorImageInfo{colorSampler, offScreenFrameBuf.normal.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};

        vk::DescriptorImageInfo texDescriptorAlbedo =
            vk::descriptorImageInfo{colorSampler, offScreenFrameBuf.albedo.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
        ...
        writeDescriptorSets = {
            // Binding 0: Vertex shader uniform buffer
            vk::writeDescriptorSet{descriptorSets.floor, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.vsOffscreen.descriptor},
            // Binding 1: Color map
            vk::writeDescriptorSet{descriptorSets.floor, vk::DescriptorType::eCombinedImageSampler, 1, &textures.floor.colorMap.descriptor},
            // Binding 2: Normal map
            vk::writeDescriptorSet{descriptorSets.floor, vk::DescriptorType::eCombinedImageSampler, 2, &textures.floor.normalMap.descriptor}
        };
        vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
    }
