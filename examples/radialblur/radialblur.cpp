...
public:
			...
			*/
            {
                ...
                if (blur) {
                    vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.radialBlur, 0, 1, &descriptorSets.radialBlur, 0,
                                            NULL);
    ...
    void loadAssets() {
        models.example.loadFromFile(context, getAssetPath() + "models/glowsphere.dae", vertexLayout, 0.05f);
    ...
    void setupDescriptorSetLayout() {
        ...
        setLayoutBindings = { // Binding 0 : Vertex shader uniform buffer
                              { vk::DescriptorType::eUniformBuffer, vk::ShaderStageFlagBits::eFragment, 0 },
                              // Binding 0: Fragment shader image sampler
                              { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 1 }
        };
        descriptorLayout{ setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()) };
        VK_CHECK_RESULT(vkCreateDescriptorSetLayout(device, &descriptorLayout, nullptr, &descriptorSetLayouts.radialBlur));
        pipelineLayoutCreateInfo{ &descriptorSetLayouts.radialBlur, 1 };
    ...
    void setupDescriptorSet() {
        ...
        std::vector<vk::WriteDescriptorSet> offScreenWriteDescriptorSets = {
            ...
            { descriptorSets.scene, vk::DescriptorType::eCombinedImageSampler, 1, &textures.gradient.descriptor },
        };
        vkUpdateDescriptorSets(device, offScreenWriteDescriptorSets.size(), offScreenWriteDescriptorSets.data(), 0, NULL);

        // Fullscreen radial blur
        descriptorSetAllocInfo{ descriptorPool, &descriptorSetLayouts.radialBlur, 1 };
        VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &descriptorSetAllocInfo, &descriptorSets.radialBlur));

        std::vector<vk::WriteDescriptorSet> writeDescriptorSets = {
            // Binding 0: Vertex shader uniform buffer
            { descriptorSets.radialBlur, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.blurParams.descriptor },
            // Binding 0: Fragment shader texture sampler
            { descriptorSets.radialBlur, vk::DescriptorType::eCombinedImageSampler, 1, &offscreenPass.descriptor },
        };
    ...
    void preparePipelines() {
        ...
        vk::GraphicsPipelineCreateInfo pipelineCI{ pipelineLayouts.radialBlur, renderPass, 0 };
    ...
    void prepareUniformBuffers() {
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                                   ...
                                                   sizeof(uboBlurParams), &uboBlurParams));
