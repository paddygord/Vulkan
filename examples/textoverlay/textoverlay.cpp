...
public:
    ...
    TextOverlay(vks::VulkanDevice* vulkanDevice,
                vk::Queue queue,
                std::vector<vk::Framebuffer>& framebuffers,
                vk::Format colorformat,
                vk::Format depthformat,
    ...
    void prepareResources() {
        ...
        vk::CommandBufferAllocateInfo cmdBufAllocateInfo =
            vk::commandBufferAllocateInfo{commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, (uint32_t)cmdBuffers.size()};
        ...
        vk::BufferCreateInfo bufferInfo{ VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, bufferSize };
        ...
        setLayoutBindings[0]{ vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 0 };
        ...
        // Pipeline layout
        vk::PipelineLayoutCreateInfo pipelineLayoutInfo = vk::pipelineLayoutCreateInfo{&descriptorSetLayout, 1};
        VK_CHECK_RESULT(vkCreatePipelineLayout(vulkanDevice->logicalDevice, &pipelineLayoutInfo, nullptr, &pipelineLayout));

        // Descriptor set
        vk::DescriptorSetAllocateInfo descriptorSetAllocInfo = vk::descriptorSetAllocateInfo{descriptorPool, &descriptorSetLayout, 1};

        VK_CHECK_RESULT(vkAllocateDescriptorSets(vulkanDevice->logicalDevice, &descriptorSetAllocInfo, &descriptorSet));

        vk::DescriptorImageInfo texDescriptor = vk::descriptorImageInfo{sampler, view, VK_IMAGE_LAYOUT_GENERAL};

        std::array<vk::WriteDescriptorSet, 1> writeDescriptorSets;
        writeDescriptorSets[0]{ descriptorSet, vk::DescriptorType::eCombinedImageSampler, 0, &texDescriptor };
    ...
    void preparePipeline() {
        ...
        vk::PipelineInputAssemblyStateCreateInfo inputAssemblyState{ VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP, 0, VK_FALSE };
        ...
        std::array<vk::VertexInputAttributeDescription, 2> vertexInputAttributes = {
            { 0, 0, vk::Format::eR32G32sFloat, 0 },                  // Location 0: Position
            { 1, 1, vk::Format::eR32G32sFloat, sizeof(glm::vec2) },  // Location 1: UV
        };
    ...
    void updateCommandBuffers() {
        ...
        for (int32_t i = 0; i < cmdBuffers.size(); ++i) {
            ...
            vk::Viewport viewport{ (float)*frameBufferWidth, (float)*frameBufferHeight, 0.0f, 1.0f };
            vkCmdSetViewport(cmdBuffers[i], 0, 1, &viewport);

            vk::Rect2D scissor{ *frameBufferWidth, *frameBufferHeight, 0, 0 };
...
public:
        ...
        for (int32_t x = -1; x <= 1; x += 2) {
            for (int32_t y = -1; y <= 1; y += 2) {
                for (int32_t z = -1; z <= 1; z += 2) {
                    ...
                    glm::vec3 projected =
                        glm::project(glm::vec3((float)x, (float)y, (float)z), uboVS.model, uboVS.projection, glm::vec4(0, 0, (float)width, (float)height));
    ...
    void preparePipelines() {
        ...
        std::vector<vk::VertexInputAttributeDescription> vertexInputAttributes = {
            ...
            { 0, 2, vk::Format::eR32G32sFloat, sizeof(float) * 6 },     // Location 2: Texture coordinates
            { 0, 3, vk::Format::eR32G32B32sFloat, sizeof(float) * 8 },  // Location 3: Color
        };
        vk::PipelineVertexInputStateCreateInfo vertexInputState;
        vertexInputState.vertexBindingDescriptionCount = static_cast<uint32_t>(vertexInputBindings.size());
        vertexInputState.pVertexBindingDescriptions = vertexInputBindings.data();
