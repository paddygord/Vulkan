...
public:
    struct {
        vkx::texture::Texture2D ssaoNoise;
    } textures;
    ...
    void createAttachment(vk::Format format, vk::ImageUsageFlagBits usage, FrameBufferAttachment* attachment, uint32_t width, uint32_t height) {
    ...
    void prepareOffscreenFramebuffers() {
        ...
        // G-Buffer
        createAttachment(vk::Format::eR32G32B32A32sFloat, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.offscreen.position, width,
                         height);                                                                                                          // Position + Depth
        createAttachment(vk::Format::eR8G8B8A8Unorm, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.offscreen.normal, width, height);  // Normals
        createAttachment(vk::Format::eR8G8B8A8Unorm, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.offscreen.albedo, width, height);  // Albedo (color)
        ...
        // SSAO
        createAttachment(vk::Format::eR8Unorm, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.ssao.color, ssaoWidth, ssaoHeight);  // Color

        // SSAO blur
        createAttachment(vk::Format::eR8Unorm, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, &frameBuffers.ssaoBlur.color, width, height);  // Color
			...
			*/
            {
				...
				*/
                ...
                vk::Viewport viewport{ (float)frameBuffers.offscreen.width, (float)frameBuffers.offscreen.height, 0.0f, 1.0f };
                vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

                vk::Rect2D scissor{ frameBuffers.offscreen.width, frameBuffers.offscreen.height, 0, 0 };
				...
				*/
                ...
                viewport{ (float)frameBuffers.ssao.width, (float)frameBuffers.ssao.height, 0.0f, 1.0f };
                vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);
                scissor{ frameBuffers.ssao.width, frameBuffers.ssao.height, 0, 0 };
                vkCmdSetScissor(drawCmdBuffers[i], 0, 1, &scissor);
				...
				*/
                ...
                viewport{ (float)frameBuffers.ssaoBlur.width, (float)frameBuffers.ssaoBlur.height, 0.0f, 1.0f };
                vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);
                scissor{ frameBuffers.ssaoBlur.width, frameBuffers.ssaoBlur.height, 0, 0 };
                vkCmdSetScissor(drawCmdBuffers[i], 0, 1, &scissor);
    ...
    void setupDescriptorPool() {
        std::vector<vk::DescriptorPoolSize> poolSizes = { { vk::DescriptorType::eUniformBuffer, 10 }, { vk::DescriptorType::eCombinedImageSampler, 12 } };
        vk::DescriptorPoolCreateInfo descriptorPoolInfo{ poolSizes, descriptorSets.count };
    ...
    void setupLayoutsAndDescriptors() {
        ...
        vk::DescriptorSetAllocateInfo descriptorAllocInfo{ descriptorPool, nullptr, 1 };
        std::vector<vk::WriteDescriptorSet> writeDescriptorSets;
        std::vector<vk::DescriptorImageInfo> imageDescriptors;

        // G-Buffer creation (offscreen scene rendering)
        setLayoutBindings = {
            { vk::DescriptorType::eUniformBuffer, vk::ShaderStageFlagBits::eVertex, 0 },           // VS UBO
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 1 },  // FS Color
        };
        setLayoutCreateInfo{ setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()) };
        ...
        writeDescriptorSets = {
            { descriptorSets.floor, vk::DescriptorType::eUniformBuffer, 0, &uniformBuffers.sceneMatrices.descriptor },
        };
        vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);

        // SSAO Generation
        setLayoutBindings = {
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 0 },  // FS Position+Depth
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 1 },  // FS Normals
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 2 },  // FS SSAO Noise
            { vk::DescriptorType::eUniformBuffer, vk::ShaderStageFlagBits::eFragment, 3 },         // FS SSAO Kernel UBO
            { vk::DescriptorType::eUniformBuffer, vk::ShaderStageFlagBits::eFragment, 4 },         // FS Params UBO
        };
        setLayoutCreateInfo{ setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()) };
        ...
        imageDescriptors = {
            { colorSampler, frameBuffers.offscreen.position.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
            { colorSampler, frameBuffers.offscreen.normal.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
        };
        writeDescriptorSets = {
            { descriptorSets.ssao, vk::DescriptorType::eCombinedImageSampler, 0, &imageDescriptors[0] },            // FS Position+Depth
            { descriptorSets.ssao, vk::DescriptorType::eCombinedImageSampler, 1, &imageDescriptors[1] },            // FS Normals
            { descriptorSets.ssao, vk::DescriptorType::eCombinedImageSampler, 2, &textures.ssaoNoise.descriptor },  // FS SSAO Noise
            { descriptorSets.ssao, vk::DescriptorType::eUniformBuffer, 3, &uniformBuffers.ssaoKernel.descriptor },  // FS SSAO Kernel UBO
            { descriptorSets.ssao, vk::DescriptorType::eUniformBuffer, 4, &uniformBuffers.ssaoParams.descriptor },  // FS SSAO Params UBO
        };
        vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);

        // SSAO Blur
        setLayoutBindings = {
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 0 },  // FS Sampler SSAO
        };
        setLayoutCreateInfo{ setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()) };
        ...
        imageDescriptors = {
            { colorSampler, frameBuffers.ssao.color.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
        };
        writeDescriptorSets = {
            { descriptorSets.ssaoBlur, vk::DescriptorType::eCombinedImageSampler, 0, &imageDescriptors[0] },
        };
        vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
        ...
        setLayoutBindings = {
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 0 },  // FS Position+Depth
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 1 },  // FS Normals
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 2 },  // FS Albedo
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 3 },  // FS SSAO
            { vk::DescriptorType::eCombinedImageSampler, vk::ShaderStageFlagBits::eFragment, 4 },  // FS SSAO blurred
            { vk::DescriptorType::eUniformBuffer, vk::ShaderStageFlagBits::eFragment, 5 },         // FS Lights UBO
        };
        setLayoutCreateInfo{ setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()) };
        ...
        imageDescriptors = {
            { colorSampler, frameBuffers.offscreen.position.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
            { colorSampler, frameBuffers.offscreen.normal.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
            { colorSampler, frameBuffers.offscreen.albedo.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
            { colorSampler, frameBuffers.ssao.color.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
            { colorSampler, frameBuffers.ssaoBlur.color.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL },
        };
        writeDescriptorSets = {
            { descriptorSets.composition, vk::DescriptorType::eCombinedImageSampler, 0, &imageDescriptors[0] },            // FS Sampler Position+Depth
            { descriptorSets.composition, vk::DescriptorType::eCombinedImageSampler, 1, &imageDescriptors[1] },            // FS Sampler Normals
            { descriptorSets.composition, vk::DescriptorType::eCombinedImageSampler, 2, &imageDescriptors[2] },            // FS Sampler Albedo
            { descriptorSets.composition, vk::DescriptorType::eCombinedImageSampler, 3, &imageDescriptors[3] },            // FS Sampler SSAO
            { descriptorSets.composition, vk::DescriptorType::eCombinedImageSampler, 4, &imageDescriptors[4] },            // FS Sampler SSAO blurred
            { descriptorSets.composition, vk::DescriptorType::eUniformBuffer, 5, &uniformBuffers.ssaoParams.descriptor },  // FS SSAO Params UBO
        };
        vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);
    }
    ...
    void preparePipelines() {
        ...
        // SSAO Pass
        {
            ...
            specializationMapEntries[0]{ 0, 0, sizeof(uint32_t) };              // SSAO Kernel size
            specializationMapEntries[1]{ 1, sizeof(uint32_t), sizeof(float) };  // SSAO radius
            struct {
    ...
    void prepareUniformBuffers() {
        ...
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformBuffers.sceneMatrices, sizeof(uboSceneMatrices));

        // SSAO parameters
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformBuffers.ssaoParams, sizeof(uboSSAOParams));
        ...
        // Upload as UBO
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformBuffers.ssaoKernel, ssaoKernel.size() * sizeof(glm::vec4), ssaoKernel.data());
        ...
        // Upload as texture
        textures.ssaoNoise.fromBuffer(ssaoNoise.data(), ssaoNoise.size() * sizeof(glm::vec4), vk::Format::eR32G32B32A32sFloat, SSAO_NOISE_DIM, SSAO_NOISE_DIM,
